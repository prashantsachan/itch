...
public class MaxFeeTxHandler {
	...
	protected Transaction getTransactionToAdd(Transaction[] possibleTxs) {
		...
		int validIdx = -1;
		double maxCoinDiff = -1; 
		for(int i=0;i< possibleTxs.length;i++) {
			if(isValidTx(possibleTxs[i]) && coinDiff(possibleTxs[i]) > maxCoinDiff) {
				validIdx = i;
			}
		}
		if(validIdx<0)
	...
	private double coinDiff(Transaction tx) {
		double diff = 0;
		for(Transaction.Input txInput : tx.getInputs()) {
			UTXO utxo = new UTXO(txInput.prevTxHash, txInput.outputIndex);
			diff+= pool.getTxOutput(utxo).value;
		}
		for(Transaction.Output txOutput: tx.getOutputs())
			diff-= txOutput.value;
		return diff;
